        ...
        <div>
            ...
            <div class="flex flex-col gap-4 py-4  lg:flex-row">
                ...
                <div class="flex h-56 w-full flex-col items-center justify-center gap-4 rounded-xl border border-dashed border-gray-300 p-5 text-center">
                    <img id="oldImg" src="{{asset('backend/images/default.jpg')}}" class="h-16 w-16 rounded-full" alt="Img">
                    ...
                    <input oninput="oldImg.src=window.URL.createObjectURL(this.files[0])"  type="file" class="pb-4 form-control" id="productImgUpdate">
...
<script>
    ...
    async function getProfileAccount(){
        ...
        try{
            ...
            if(res.status===200){
                ...
                document.getElementById('filePath').value=res.data['file_path'];
                // document.getElementById('oldImg').src=res.data['file_path'];
            }
            else{
    ...
    async function onUpdateAccount(){
        ...
        //     "img":document.getElementById('filePath').value,
        // }
        ...
        showLoader();
        let res=await axios.post("/user-update",formData,config,HeaderToken());
        hideLoader();
